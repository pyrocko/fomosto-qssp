...
c
      integer i,j,key
      double complex cldeg,cllm1,cllp1,c2lp1,c2lm1,c2lp3,cxp2,acc
      double complex ph0jup,ph1jup,ph0yup,ph1yup
      double complex ph0jlw,ph1jlw,ph0ylw,ph1ylw
      double complex spb(2,2),mas(4,4)
...
c
      do j=3,4
        do i=1,4
          mas4x4lw(i,j,ly)=mas4x4up(i,j,ly)
        enddo
      enddo
c
      if(lwup.eq.0)then
...
c
      ...
      if(key.eq.0)then
        print *,' Warning in qpsmatc: anormal exit from cdgemp!'
        return
      endif
c
      return
